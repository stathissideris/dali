(ns dali.core
  (:require [dali.math :refer [cos sin minus polar-angle abs
                               sqrt solve-2x2-system
                               linear-equation-at-x within]]
            [dali.utils :refer [num-or-fn? coll-of-nums-or-fns?
                                function? match-non-match
                                exception]])
  (#+clj :use #+cljs :use-macros [dali.macros :only [defshape dynamic]]))
...
(let [transform-directives #{:scale :skew :translate :rotate :matrix}]
  ...
  (defn transform-spec
    ...
    (if-not (validate-transform-spec spec)
      (throw (exception "Transform spec " spec " is not valid."))
      spec)))
